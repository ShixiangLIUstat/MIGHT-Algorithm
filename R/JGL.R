...
Calculate_ebic <- function(Theta, data_list, n, p, gamma) {
  ...
  for (k in 1:K) {
    ...
    temp2 <- log(abs(det(theta)))
    log_likelihood <- ifelse(abs(temp2)>1e4, 1e4, temp2) - sum(diag(S %*% theta))
...
JointGrpah_BIC <- function(X, gamma){
  ...
  for (i in seq_along(lambda)) {
      ...
      fit <- jointgraph_train(X_list, Y, lambda_value = lambda[i], adaptive_weight=array(1, c(length(unique(Y)), ncol(X_list), ncol(X_list))))
...
JGL_EBIC <- function(X, penalty = "group", tol = 1e-3, maxiter = 200, gamma = 0) {
  ...
  # cat("Optimal lambda2:", optimal_lambda2, "\n")


  best_fit <- JGL(Y = X, penalty = "group", lambda1 = optimal_lambda1, lambda2 = optimal_lambda2,  return.whole.theta = T, tol = 1e-4)
  return(best_fit)
}
...
fasjem_EBIC <- function(X, lambda_grid = 10^( seq(-2, 1, length.out = 5) ),
  ...
  for (i in seq_along(lambda_grid)) {
    for (j in seq_along(epsilon_grid)) {
      ...
      result <- fasjem(lapply(X, cov), lambda = lambda, epsilon = epsilon,
                       gamma= lambda, rho= 1)
      # result <- fasjem(X, method = "fasjem_g", lambda = 0.1, epsilon =0.1, gamma = 0.1, rho = 0.1)
      # calculate_ebic(result, X, n, p)
      for (k in 1:length(X)) {
        if (any(is.na(result[[k]]))) {
        result[[k]][which(is.na(result[[k]]))] <- 0
        }
      }
  ...
  result <- fasjem(lapply(X, cov), lambda = optimal_lambda, gamma= optimal_lambda,
